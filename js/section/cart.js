...
function renderProduct(arr) {
  ...
  setTimeout(() => {
    ...
    if (arr.length === 0) {
      ...
      html = arr.map((product, index) => `
      <li class="electronik_list_item">
        <div class="empty-item_content">
          <img class="empty-product_img" src="${product.IMG.img_1.split(' ')[0]}" alt="">
          <svg data-id="${index}" class="empty-product_favorites" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
            <path
